...
public class Exploder extends Transformer implements MLWritable {
    ...
    public Dataset<Row> transform(Dataset<?> sentences) {
        ...
        org.apache.spark.sql.Column singular = explode(column(Column.FILTERED_WORDS.getName()))
                .as(Column.FILTERED_WORD.getName());
        return sentences.select(column(Column.ID.getName()), column(Column.ROW_NUMBER.getName()),
                column(Column.LABEL.getName()), singular);
    }
