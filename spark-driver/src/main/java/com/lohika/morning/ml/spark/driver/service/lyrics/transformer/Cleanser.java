...
public class Cleanser extends Transformer implements MLWritable {
    ...
    public Dataset<Row> transform(Dataset<?> sentences) {
        ...
        sentences = sentences.withColumn(Column.CLEAN.getName(),
                regexp_replace(trim(column(Column.VALUE.getName())), "[^\\w\\s]", ""));
        sentences = sentences.drop(Column.VALUE.getName());

        // Remove double spaces.
        return sentences.withColumn(Column.CLEAN.getName(),
                regexp_replace(column(Column.CLEAN.getName()), "\\s{2,}", " "));
    }
